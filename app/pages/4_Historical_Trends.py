...
def load_rainfall_history():
    ...
    return df[df["SUBDIVISION"].str.upper() == "KERALA"]
...
with col1:
    fig1 = px.line(kerala_df, x="YEAR", y="ANNUAL", template=plotly_template(),
                    title="Annual Rainfall Over Time", labels={"ANNUAL": "Rainfall (mm)", "YEAR": "Year"})
...
with col2:
    fig2 = px.line(kerala_df, x="YEAR", y="Jun-Sep", template=plotly_template(),
                    title="Monsoon Season Rainfall (Jun–Sep)", labels={"Jun-Sep": "Rainfall (mm)", "YEAR": "Year"})
    fig2.update_traces(line_color="#F97316")
    fig2.add_vline(x=2018, line_dash="dot", annotation_text="2018 Floods", line_color="#EF4444")
...
fig3 = px.bar(x=monthly_avg.index, y=monthly_avg.values, template=plotly_template(),
